...
export const sendAnswersToEmail = async (email, destinations) => {
  try {
    ...
    if (!res.ok) {
      throw new Error("Failed to schedule follow-up email:", error);
    }
...
export const sendThankYouEmail = async (email) => {
  try {
    const res = await fetch("/api/send-thankyou-email", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    if (!res.ok) {
      ...
      throw new Error(error.error || "Failed to send thank-you email");
    }
